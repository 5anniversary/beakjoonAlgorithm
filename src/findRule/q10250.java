/*
    ...
    문제 : ACM 호텔 매니저 지우는 손님이 도착하는 대로 빈 방을 배정하고 있다. 고객 설문조사에 따르면 손님들은 호텔 정문으로부터 걸어서 가장 짧은 거리에 있는 방을 선호한다고 한다.
          ...
          즉, 그림 1 에서 빗금으로 표시한 방은 305 호가 된다. 손님은 엘리베이터를 타고 이동하는 거리는 신경 쓰지 않는다. 다만 걷는 거리가 같을 때에는 아래층의 방을 더 선호한다.
...
public class q10250 {
    public static void main(String[] args) {
        ...
        while (T != 0) {
            int floor = scan.nextInt(), room = scan.nextInt(), ticket = scan.nextInt(), i = 0, j = 0, cnt = 1;
            for (i = 1; i <= room; i++) {
                for (j = 1; j < floor; j++) {
                    if (cnt == ticket)
                        break;
...
/*
    static int findRoom(int floor, int room, int ticket) {
        int i=0, j=0, cnt=1;
        for (i = 1; i < room; i++) {
            for (j = 1; j < floor; j++) {
                if(cnt==ticket)
    ...
    public static void main(String[] args) {
        ...
        for(int num = 0; num < test; num++){
            String floor, room, ticket;
            for (int i = 0; i < test; ++i) {
                floor = scan.next();
                room = scan.next();
                ticket = scan.next();
                int floor_int = Integer.parseInt(floor);
                int room_int = Integer.parseInt(room);
                int ticket_int = Integer.parseInt(ticket);
                System.out.println(findRoom(floor_int, room_int, ticket_int));
            }
        }
    }*/